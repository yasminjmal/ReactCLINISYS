...
import { CheckCircle, Search, ArrowUpDown, Info, Tag, CalendarDays, User, Layers } from 'lucide-react';

const TicketResoluCard = ({ ticket, onVoirDetails }) => {
    ...
    <div className="bg-white dark:bg-slate-800 shadow-lg rounded-lg p-5 border border-slate-200 dark:border-slate-700 hover:shadow-xl transition-shadow duration-300 flex flex-col justify-between">
      <div>
        <div className="flex justify-between items-start mb-3">
          <h3 className="text-lg font-semibold text-green-600 dark:text-green-400 truncate" title={ticket.titre}>
            {ticket.titre}
          </h3>
        ...
        <div className="grid grid-cols-2 gap-x-4 gap-y-2 text-xs text-slate-500 dark:text-slate-400 mb-4">
          ...
          <div className="flex items-center" title="Date de résolution">
            <CalendarDays size={14} className="mr-1.5 text-slate-400 dark:text-slate-500" /> 
            Résolu le: {ticket.dateResolutionEmploye ? new Date(ticket.dateResolutionEmploye).toLocaleDateString() : 'N/A'}
          </div>
          {ticket.demandeur && (
      ...
      <div className="mt-auto pt-4 border-t border-slate-100 dark:border-slate-700/50 flex justify-end">
        <button
          ...
          title="Voir les détails de ce ticket résolu"
        >
...
const MesTicketsResolusPage = ({ ticketsTermines, onVoirDetailsTicketCallback }) => {
  const [searchTerm, setSearchTerm] = useState('');
  const [sortConfig, setSortConfig] = useState({ key: 'dateResolutionEmploye', direction: 'descending' });

  const sortedAndFilteredTickets = useMemo(() => {
    if (!Array.isArray(ticketsTermines)) return [];
    let sortableTickets = [...ticketsTermines];
    ...
    if (searchTerm) {
      sortableTickets = sortableTickets.filter(ticket =>
        ...
        (ticket.notesResolutionEmploye && ticket.notesResolutionEmploye.toLowerCase().includes(searchTerm.toLowerCase())) ||
    ...
    if (sortConfig.key) {
      sortableTickets.sort((a, b) => {
        ...
        if (['dateResolutionEmploye', 'dateCreation', 'dateAssignationEmploye'].includes(sortConfig.key)) {
  ...
  }, [ticketsTermines, searchTerm, sortConfig]);
  ...
  if (!Array.isArray(ticketsTermines) || ticketsTermines.length === 0 && !searchTerm) {
    return (
      <div className="p-6 md:p-10 text-center bg-slate-50 dark:bg-slate-800/50 rounded-lg shadow-inner">
        ...
        <p className="text-slate-500 dark:text-slate-400">Vous n'avez actuellement aucun ticket marqué comme terminé.</p>
      </div>
    );
  }
      ...
      <div className="p-4 bg-white dark:bg-slate-800 rounded-lg shadow">
        <div className="flex flex-col sm:flex-row gap-4">
          <div className="relative flex-grow">
            ...
            <input
              type="text"
              placeholder="Rechercher par titre, réf, notes, demandeur..."
              value={searchTerm}
        ...
        <div className="mt-3 text-xs text-slate-500 dark:text-slate-400 flex flex-wrap gap-x-2">
            <span>Trier par:</span>
            <button onClick={() => requestSort('dateResolutionEmploye')} className="font-medium hover:text-sky-500">Date Résolution {getSortIndicator('dateResolutionEmploye')}</button>
            <span>|</span>
      ...
      {sortedAndFilteredTickets.length > 0 ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
         ...
         <div className="p-6 md:p-10 text-center bg-slate-50 dark:bg-slate-800/50 rounded-lg shadow-inner">
            ...
            <p className="text-slate-500 dark:text-slate-400">Aucun ticket résolu ne correspond à vos critères de recherche.</p>
        </div>
      )}
      <style jsx>{`
