...
import { FileText, Search, Tag, CalendarDays, User, Layers, CheckCircle, XCircle, MoreVertical, Edit3, Save } from 'lucide-react';
...
// Composant TicketCard pour "Mon Travail en Cours"
const TicketCard = ({ ticket, onCloturerClick, onViewDetails, onUpdateEcheanceClick }) => {
...
const MonTravailEnCoursPage = () => {
  ...
  const sortedAndFilteredTickets = useMemo(() => {
    ...
    if (sortConfig.key) {
      sortableTickets.sort((a, b) => {
        ...
        if (sortConfig.key === 'dateCreation' || sortConfig.key === 'dateTraitement' || sortConfig.key === 'date_echeance') {
  ...
  return (
    <>
      <div className="space-y-6">
        <div className="p-4 bg-white dark:bg-slate-800 rounded-lg shadow">
          ...
          <div className="mt-3 text-xs text-slate-500 dark:text-slate-400 flex flex-wrap gap-x-2">
              <span>Trier par:</span>
              <button onClick={() => requestSort('dateTraitement')} className="font-medium hover:text-sky-500">Date de Traitement {getSortIndicator('dateTraitement')}</button>
              <span>|</span>
      ...
      {/* Modal pour la clôture du ticket */}
      <Modal
      ...
      >
        <p className="text-sm text-slate-600 dark:text-slate-300">
          Êtes-vous sûr de vouloir clôturer le ticket "<strong className="text-slate-700 dark:text-slate-100">{tickets.find(t => t.id === ticketACloturerId)?.titre}</strong>" (ID: {ticketACloturerId}) ?
          <br/>Son statut passera à "Terminé".
        </p>
      </Modal>

      {/* Modal de modification date d'échéance */}
      <Modal
      ...
      {/* Modal de détails du ticket avec commentaires et documents */}
      <Modal
