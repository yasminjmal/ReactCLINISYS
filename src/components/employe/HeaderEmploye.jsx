// src/components/employe/NavbarEmploye.jsx
import React, { useState, useEffect, useRef } from 'react';
import { AlignJustify, Bell, ChevronDown, UserCircle, LogOut } from 'lucide-react';
...
const NavbarEmploye = ({ USER,toggleSidebar, onNavigateToProfile }) => {
    ...
    const userName = user ? `${user.prenom || ''} ${user.nom || ''}`.trim() : 'Employé';
    ...
    return (
        <nav className="bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800 p-4 flex items-center justify-between shadow-sm sticky top-0 z-40">
            ...
            <div className="flex items-center gap-4 md:gap-6 ml-auto">
                {/* Le bouton de thème pourrait être ajouté ici si nécessaire */}
                ...
                <div className="relative" ref={dropdownRef}>
                    ...
                    {dropdownOpen && (
                        <div className="absolute right-0 mt-2 w-48 bg-white dark:bg-slate-800 rounded-md shadow-lg py-1 border border-slate-200 dark:border-slate-700">
                            ...
                            <button onClick={() => { onNavigateToProfile(); setDropdownOpen(false); }} className="w-full text-left flex items-center gap-3 px-4 py-2 text-sm text-slate-700 dark:text-slate-200 hover:bg-slate-100 dark:hover:bg-slate-700">
