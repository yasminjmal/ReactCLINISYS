...
// import { Search, List, LayoutGrid, Filter, ArrowUpDown, ChevronDown, ChevronUp, Briefcase, Plus, X, Edit, Trash2, Save, XCircle, AlertTriangle, Table, CheckCircle2, RefreshCw, Tag, ToggleLeft, UserSquare, CalendarDays } from 'lucide-react';
// // Assurez-vous que le chemin d'importation vers votre service est correct
// import posteService from '../../../services/posteService'; 

// // --- COMPOSANTS ENFANTS (inclus pour la simplicité, peuvent être dans des fichiers séparés) ---
...
// const PostesTable = ({ postes, onEdit, onDelete, newlyAddedId }) => ( <div className="overflow-x-auto bg-white dark:bg-slate-800 rounded-lg shadow"><table className="min-w-full divide-y divide-slate-200 dark:divide-slate-700"><thead className="bg-slate-50 dark:bg-slate-700/50"><tr><th scope="col" className="th-cell"><Tag className="inline-block mr-2 h-4 w-4"/>Poste</th><th scope="col" className="th-cell"><ToggleLeft className="inline-block mr-2 h-4 w-4"/>Statut</th><th scope="col" className="th-cell"><UserSquare className="inline-block mr-2 h-4 w-4"/>Créé par</th><th scope="col" className="th-cell"><CalendarDays className="inline-block mr-2 h-4 w-4"/>Date création</th><th scope="col" className="relative px-6 py-3"><span className="sr-only">Actions</span></th></tr></thead><tbody className="bg-white dark:bg-slate-800 divide-y divide-slate-200 dark:divide-slate-700">{postes.map((p) => ( <tr key={p.id} className={`tr-hover ${p.id === newlyAddedId ? 'highlight-new' : ''}`}><td className="td-cell font-medium text-slate-900 dark:text-slate-100">{p.designation}</td><td className="td-cell"><span className={`status-badge ${p.actif ? 'status-green' : 'status-red'}`}>{p.actif ? <CheckCircle2 size={14} className="mr-1.5"/> : <XCircle size={14} className="mr-1.5"/>}{p.actif ? 'Actif' : 'Inactif'}</span></td><td className="td-cell text-slate-500 dark:text-slate-400">{p.userCreation}</td><td className="td-cell text-slate-500 dark:text-slate-400">{formatDate(p.dateCreation)}</td><td className="td-cell text-right"><div className="flex items-center justify-end space-x-4"><button onClick={() => onEdit(p)} className="action-btn text-sky-600" title="Modifier"><Edit size={18} /></button><button onClick={() => onDelete(p)} className="action-btn text-red-600" title="Supprimer"><Trash2 size={18} /></button></div></td></tr>))}</tbody></table></div>);
// const PosteRow = ({ poste, onEdit, onDelete, newlyAddedId }) => ( <div className={`bg-white dark:bg-slate-800 shadow-sm hover:shadow-md rounded-lg p-4 flex items-center space-x-4 transition-all duration-200 ${poste.id === newlyAddedId ? 'highlight-new' : ''}`}><div className="flex-none p-2 bg-amber-100 dark:bg-amber-700/30 rounded-lg"><Briefcase className="text-amber-600 dark:text-amber-400" size={24} /></div><div className="flex-grow grid grid-cols-1 md:grid-cols-3 gap-4 items-center"><div className="truncate"><p className="text-md font-semibold text-slate-800 dark:text-slate-100 truncate" title={poste.designation}>{poste.designation}</p><span className={`status-badge ${poste.actif ? 'status-green' : 'status-red'}`}>{poste.actif ? <CheckCircle2 size={14} className="mr-1.5"/> : <XCircle size={14} className="mr-1.5"/>}{poste.actif ? 'Actif' : 'Inactif'}</span></div><div className="text-sm text-slate-500 dark:text-slate-400"><p>Créé par : <span className="font-medium text-slate-700 dark:text-slate-300">{poste.userCreation}</span></p></div><div className="text-sm text-slate-500 dark:text-slate-400"><p>Le : <span className="font-medium text-slate-700 dark:text-slate-300">{formatDate(poste.dateCreation)}</span></p></div></div><div className="flex items-center space-x-2"><button onClick={() => onEdit(poste)} className="action-btn text-sky-600" title="Modifier"><Edit size={18}/></button><button onClick={() => onDelete(poste)} className="action-btn text-red-600" title="Supprimer"><Trash2 size={18}/></button></div></div>);
// const PosteCard = ({ poste, onEdit, onDelete, newlyAddedId }) => ( <div className={`bg-white dark:bg-slate-800 rounded-xl shadow-lg p-5 flex flex-col relative transition-all duration-300 hover:shadow-2xl hover:-translate-y-1 ${poste.id === newlyAddedId ? 'highlight-new' : ''}`}><div className="flex-grow mb-4"><div className="flex items-start justify-between mb-2"><div className="flex items-center"><div className="p-2 bg-amber-100 dark:bg-amber-700/30 rounded-lg mr-3"><Briefcase className="text-amber-600 dark:text-amber-400" size={20} /></div><h3 className="text-lg font-bold text-slate-800 dark:text-slate-100">{poste.designation}</h3></div><span className={`status-badge ${poste.actif ? 'status-green' : 'status-red'}`}>{poste.actif ? <CheckCircle2 size={14} className="mr-1.5"/> : <XCircle size={14} className="mr-1.5"/>}{poste.actif ? 'Actif' : 'Inactif'}</span></div><div className="text-sm text-slate-500 dark:text-slate-400 space-y-1 pl-12"><p>Créé par : <span className="font-medium text-slate-600 dark:text-slate-300">{poste.userCreation}</span></p><p>Le : <span className="font-medium text-slate-600 dark:text-slate-300">{formatDate(poste.dateCreation)}</span></p></div></div><div className="flex justify-end space-x-2 border-t border-slate-100 dark:border-slate-700 pt-3"><button onClick={() => onEdit(poste)} className="btn btn-secondary-outline btn-sm group"><Edit size={16} className="mr-1.5"/> Modifier</button><button onClick={() => onDelete(poste)} className="btn btn-danger-outline btn-sm group"><Trash2 size={16} className="mr-1.5"/> Supprimer</button></div></div>);
...
//         try {
//             let params = {};
//             if (statusFilter === 'actif') params.actifs = [true];
//             if (statusFilter === 'inactif') params.actifs = [false];
//             const response = await posteService.getAllPostes(params);
//             setPostes(response.data);
//         } catch (err) {
//             setError('Erreur de récupération des postes depuis le serveur.');
//             console.error(err);
//         } finally {
//             setLoading(false);
//         }
...
//         let filtered = Array.isArray(postes) ? [...postes] : [];
//         if (searchTerm) {
...
//             if (activeSort.field === 'dateCreation') {
...
//         });
//         return filtered;
//     }, [postes, searchTerm, activeSort]);

//     const showMessage = (text, type = 'success', newId = null) => {
//         setPageMessage({ text, type });
//         if (newId) {
//             setNewlyAddedId(newId);
//             setTimeout(() => setNewlyAddedId(null), 3000);
//         }
//         setTimeout(() => setPageMessage({ text: '', type: '' }), 4000);
//     };

//     const handleAddClick = () => { setSelectedPoste(null); setAddOrEditModalOpen(true); };
//     const handleEditClick = (poste) => { setSelectedPoste(poste); setAddOrEditModalOpen(true); };
//     const handleDeleteClick = (poste) => { setSelectedPoste(poste); setIsDeleteModalOpen(true); };

//     const handleSave = async (data) => {
//         try {
//             if (selectedPoste) {
//                 await posteService.updatePoste(selectedPoste.id, { ...selectedPoste, ...data });
//                 showMessage('Poste mis à jour !', 'success', selectedPoste.id);
//             } else {
//                 const response = await posteService.createPoste(data);
//                 showMessage('Poste créé avec succès !', 'success', response.data.id);
//             }
...
//         try {
//             await posteService.deletePoste(selectedPoste.id);
//             showMessage('Poste supprimé.');
...
//                 {pageMessage.text && (<div className={`fixed top-20 right-6 p-4 rounded-md shadow-lg z-[100] text-white flex items-center space-x-3 animate-slide-in-right ${pageMessage.type === 'success' ? 'bg-green-500' : 'bg-red-500'}`}><span>{pageMessage.text}</span><button onClick={() => setPageMessage({ text: '', type: '' })} className="p-1 hover:bg-black/20 rounded-full"><X size={16}/></button></div>)}
//                 <div className="bg-white dark:bg-slate-800 p-4 rounded-xl shadow-md">
//                     <div className="flex flex-wrap items-center justify-between gap-4 mb-4">
//                         <div className="flex items-center"><h1 className="text-2xl font-bold text-slate-800 dark:text-slate-100">Gestion des Postes</h1><span className="ml-3 bg-slate-200 dark:bg-slate-700 text-slate-600 dark:text-slate-300 text-sm font-medium me-2 px-2.5 py-0.5 rounded-full">{processedPostes.length}</span></div>
//                         <div className="flex items-center space-x-2"><button onClick={handleAddClick} className="btn btn-primary group"><Plus size={18} className="mr-2" />Ajouter</button><button onClick={fetchPostes} className="p-2 rounded-md bg-slate-200 dark:bg-slate-700 text-slate-600 dark:text-slate-300 hover:bg-slate-300 dark:hover:bg-slate-600" title="Rafraîchir"><RefreshCw size={20} className={loading ? 'animate-spin' : ''}/></button><button onClick={() => setViewMode('table')} className={`view-btn ${viewMode === 'table' && 'active'}`} title="Vue Tableau"><Table size={20}/></button><button onClick={() => setViewMode('list')} className={`view-btn ${viewMode === 'list' && 'active'}`} title="Vue Liste"><List size={20}/></button><button onClick={() => setViewMode('grid')} className={`view-btn ${viewMode === 'grid' && 'active'}`} title="Vue Grille"><LayoutGrid size={20}/></button></div>
//                     </div>
//                     <div className="grid grid-cols-1 md:grid-cols-3 gap-4 items-center">
//                         <div className="relative"><div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none"><Search className="h-5 w-5 text-slate-400" /></div><input type="text" placeholder="Rechercher..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="form-input w-full pl-10"/></div>
//                         <div className="relative" ref={filterDropdownRef}><button onClick={() => setIsFilterDropdownOpen(!isFilterDropdownOpen)} className="form-input w-full flex items-center justify-between"><div className="flex items-center space-x-2"><Filter size={16} /><span>Filtrer par statut</span></div><ChevronDown size={16} className={`transition-transform ${isFilterDropdownOpen && "rotate-180"}`} /></button>{isFilterDropdownOpen && ( <div className="dropdown-menu"><button onClick={() => { setStatusFilter('all'); setIsFilterDropdownOpen(false); }} className="dropdown-item">Tous les postes</button><button onClick={() => { setStatusFilter('actif'); setIsFilterDropdownOpen(false); }} className="dropdown-item">Postes actifs</button><button onClick={() => { setStatusFilter('inactif'); setIsFilterDropdownOpen(false); }} className="dropdown-item">Postes inactifs</button></div>)}</div>
//                         <div className="relative" ref={sortDropdownRef}><button onClick={() => setIsSortDropdownOpen(!isSortDropdownOpen)} className="form-input w-full flex items-center justify-between"><div className="flex items-center space-x-2"><ArrowUpDown size={16} /><span>Trier par: {sortOptions.find(o => o.value === activeSort.field)?.label}</span></div><ChevronDown size={16} className={`transition-transform ${isSortDropdownOpen && "rotate-180"}`} /></button>{isSortDropdownOpen && ( <div className="dropdown-menu">{sortOptions.map(option => ( <button key={option.value} onClick={() => { setActiveSort(prev => ({ field: option.value, order: prev.field === option.value && prev.order === 'asc' ? 'desc' : 'asc' })); setIsSortDropdownOpen(false); }} className="dropdown-item justify-between"><span>{option.label}</span>{activeSort.field === option.value && (activeSort.order === 'asc' ? <ChevronUp size={14}/> : <ChevronDown size={14}/>)}</button>))}</div>)}</div>
//                     </div>
//                 </div>
//                 {loading ? <div className="text-center py-20 text-slate-500 dark:text-slate-400">Chargement...</div> : error ? <div className="text-center py-20 text-red-500">{error}</div> : processedPostes.length === 0 ? <div className="text-center py-20 text-slate-500 dark:text-slate-400"><h3 className="text-lg font-semibold">Aucun poste trouvé</h3><p>Essayez de modifier vos filtres ou <button onClick={handleAddClick} className="text-sky-500 hover:underline">créez un nouveau poste</button>.</p></div> : (<> {viewMode === 'table' && <PostesTable postes={processedPostes} onEdit={handleEditClick} onDelete={handleDeleteClick} newlyAddedId={newlyAddedId} />} {viewMode === 'list' && <div className="space-y-4">{processedPostes.map(p => <PosteRow key={p.id} poste={p} onEdit={handleEditClick} onDelete={handleDeleteClick} newlyAddedId={newlyAddedId}/>)}</div>} {viewMode === 'grid' && <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">{processedPostes.map(p => <PosteCard key={p.id} poste={p} onEdit={handleEditClick} onDelete={handleDeleteClick} newlyAddedId={newlyAddedId}/>)}</div>} </>)}
//             </div>
//             <PosteFormModal isOpen={isAddOrEditModalOpen} onClose={() => setAddOrEditModalOpen(false)} onSave={handleSave} posteToEdit={selectedPoste} />
//             <DeleteConfirmationModal isOpen={isDeleteModalOpen} onClose={() => setIsDeleteModalOpen(false)} onConfirm={confirmDelete} poste={selectedPoste} />
//             {/* Styles Globaux (inclus pour la portabilité) */}
//             <style>{`
...
//               .th-cell { @apply px-6 py-3 text-left text-xs font-medium text-slate-500 dark:text-slate-300 uppercase tracking-wider flex items-center; }
...
//               .status-badge { @apply px-2.5 py-0.5 inline-flex items-center text-xs leading-5 font-semibold rounded-full; }
...
//               @keyframes highlight { 0% { background-color: rgba(16, 185, 129, 0); } 50% { background-color: rgba(16, 185, 129, 0.2); } 100% { background-color: rgba(16, 185, 129, 0); } }
//               .highlight-new { animation: highlight 3s ease-out; }
...
// export default ConsulterPostesPage; // Décommentez cette ligne dans votre projet
