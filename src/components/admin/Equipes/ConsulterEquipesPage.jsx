...
import { Search, List, LayoutGrid, Filter, ArrowUpDown, ChevronDown, ChevronUp, Users as UsersIconPage, XCircle as AlertXCircle, CheckCircle2 as AlertCheckCircle, X as XIcon, UserPlus } from 'lucide-react';
...
const ConsulterEquipesPage = ({ 
    equipes = [], 
    users = [],
    onNavigateToAjouterEquipe, 
    onNavigateToEquipeDetails,
  ...
  const potentialChefsForFilter = safeUsers.filter(u => u.role === 'chef_equipe' || u.role === 'admin');
  const sortOptionsEquipes = [
    { value: 'dateCreation', label: 'Date Création' },
    { value: 'nom', label: 'Nom (A-Z)' },
    { value: 'nbMembres', label: 'Nb Membres' }
  ];
  ...
  useEffect(() => { /* ... (gestion highlight) ... */
    if (pageMessage && pageMessage.text && newlyAddedEquipeId) {
      setHighlightedEquipeId(newlyAddedEquipeId);
  ...
  }, [pageMessage, newlyAddedEquipeId, clearPageMessage]);
  ...
  const clearFilters = () => { /* ... */ 
    setActiveFilters({ chefEquipe: null });
    setIsFilterDropdownOpen(false);
  };

  const equipesToProcess = Array.isArray(equipes) ? equipes : [];
  let processedEquipes = [...equipesToProcess]; 
  
  if (searchTerm) {
    processedEquipes = processedEquipes.filter(equipe =>
        equipe && equipe.nom && equipe.nom.toLowerCase().includes(searchTerm.toLowerCase()) ||
        (equipe.chefEquipe && `${equipe.chefEquipe.prenom} ${equipe.chefEquipe.nom}`.toLowerCase().includes(searchTerm.toLowerCase()))
    );
  }
  if (activeFilters.chefEquipe) {
    processedEquipes = processedEquipes.filter(equipe => equipe && equipe.chefEquipe && equipe.chefEquipe.id === activeFilters.chefEquipe);
  }

  if (activeSort.field) {
    processedEquipes.sort((a, b) => { /* ... (logique de tri) ... */ 
        if (!a || !b) return 0;
        let valA, valB;
        if (activeSort.field === 'nbMembres') {
            valA = a.membres?.length || 0;
            valB = b.membres?.length || 0;
        } else {
            if (!a.hasOwnProperty(activeSort.field) || !b.hasOwnProperty(activeSort.field)) return 0;
            valA = a[activeSort.field]; 
            valB = b[activeSort.field];
        }
  ...
  const handleShowMoreMembers = (equipeId) => setExpandedMembers(prev => ({ ...prev, [equipeId]: !prev[equipeId] }));
  const countActiveFilters = () => activeFilters.chefEquipe ? 1 : 0;
    ...
    <div className="p-4 md:p-6 space-y-6 bg-slate-50 dark:bg-slate-900 min-h-full">
      {pageMessage && pageMessage.text && (
         ...
         ${pageMessage.type === 'success' ? 'bg-green-100 dark:bg-green-700 border-green-500 dark:border-green-600 text-green-700 dark:text-green-100' 
            ...
            <button onClick={clearPageMessage} className="ml-auto p-1 hover:bg-black/10 rounded-full"><XIcon size={16}/></button>
        </div>
      )}
      ...
      <div className="bg-white dark:bg-slate-800 p-4 rounded-lg shadow">
         <div className="flex flex-wrap items-center justify-between gap-4 mb-4">
          <h2 className="text-xl font-semibold text-slate-700 dark:text-slate-200">
            Consulter les Équipes ({processedEquipes.length} au total)
          </h2>
          <div className="flex items-center space-x-2">
            <button onClick={onNavigateToAjouterEquipe} className="btn btn-primary-outline group">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 items-start">
          <div className="relative">
            ...
            <input type="text" placeholder="Rechercher une équipe..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)}
          ...
          <div className="relative" ref={filterDropdownRef}>
            <button onClick={() => setIsFilterDropdownOpen(!isFilterDropdownOpen)} className="w-full flex items-center justify-between px-4 py-2 rounded-md border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-700 text-slate-700 dark:text-slate-200 hover:bg-slate-50 dark:hover:bg-slate-600">
              <div className="flex items-center space-x-2"><Filter size={18} /><span>Filtrer par Chef</span>{activeFilters.chefEquipe && (<span className="bg-sky-500 text-white text-xs font-semibold px-1.5 py-0.5 rounded-full">1</span>)}</div>
            ...
            {isFilterDropdownOpen && (
              <div className="absolute top-full left-0 mt-1 w-full md:w-72 bg-white dark:bg-slate-800 rounded-md shadow-lg border dark:border-slate-700 z-10 p-4 space-y-2">
                <h4 className="text-sm font-semibold text-slate-600 dark:text-slate-300 mb-2">Chef d'équipe</h4>
                {potentialChefsForFilter.map(chef => (
                    <label key={chef.id} className="flex items-center space-x-2 cursor-pointer text-sm text-slate-700 dark:text-slate-200">
                        <input type="radio" name="chefEquipeFilter" value={chef.id} checked={activeFilters.chefEquipe === chef.id} onChange={() => handleFilterChange('chefEquipe', chef.id)}
          ...
          <div className="relative" ref={sortDropdownRef}>
            <button onClick={() => setIsSortDropdownOpen(!isSortDropdownOpen)} className="w-full flex items-center justify-between px-4 py-2 rounded-md border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-700 text-slate-700 dark:text-slate-200 hover:bg-slate-50 dark:hover:bg-slate-600">
               <div className="flex items-center space-x-2"><ArrowUpDown size={18} /><span>Trier par: {sortOptionsEquipes.find(opt => opt.value === activeSort.field)?.label || 'Défaut'} ({activeSort.order === 'asc' ? '↑' : '↓'})</span></div>
              {isSortDropdownOpen ? <ChevronUp size={16} /> : <ChevronDown size={16} />}
            </button>
            {isSortDropdownOpen && (
              <div className="absolute top-full left-0 mt-1 w-full bg-white dark:bg-slate-800 rounded-md shadow-lg border dark:border-slate-700 z-10 py-1">
                {sortOptionsEquipes.map(option => (
      ...
      {viewMode === 'grid' ? (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-3 gap-6">
          {processedEquipes.map(equipeObj => (
            <div key={equipeObj.id} ref={equipeObj.id === newlyAddedEquipeId ? newlyAddedEquipeRef : null} 
                 className={`${equipeObj.id === highlightedEquipeId ? 'ring-2 ring-green-500 ring-offset-2 dark:ring-offset-slate-900 rounded-lg bg-green-50 dark:bg-green-500/10' : ''} transition-all duration-500`}>
              <EquipeCard 
                equipe={equipeObj}
                onShowMoreMembers={handleShowMoreMembers}
                isMembersExpanded={!!expandedMembers[equipeObj.id]}
                onNavigateToEquipeDetails={onNavigateToEquipeDetails}
                onDeleteEquipeRequest={onDeleteEquipeRequest}
              />
            </div>
          ))}
        </div>
        ...
        <div className="space-y-3">
          {processedEquipes.map(equipeObj => (
             <div key={equipeObj.id} ref={equipeObj.id === newlyAddedEquipeId ? newlyAddedEquipeRef : null}
                  className={`${equipeObj.id === highlightedEquipeId ? 'ring-2 ring-green-500 ring-offset-1 dark:ring-offset-slate-900 rounded-md bg-green-50 dark:bg-green-500/10' : ''} transition-all duration-500`}>
              <EquipeRow 
                equipe={equipeObj}
                onShowMoreMembers={handleShowMoreMembers}
                isMembersExpanded={!!expandedMembers[equipeObj.id]}
      ...
      {processedEquipes.length === 0 && (
        <p className="text-center text-slate-500 dark:text-slate-400 py-10">Aucune équipe trouvée.</p>
      )}
    </div>
  );
};
